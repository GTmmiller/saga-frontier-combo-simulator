...
describe("Skill Class", () => {
  ...
  test("a send/receive pair should combo", () => {
    expect(newSkill.canSendCombo(forwardSkill)).toBe(true);
    expect(forwardSkill.canReceiveCombo(newSkill)).toBe(true);
  });

  test("an attack-all skill can receive a combo", () => {
    expect(forwardSkill.canSendCombo(multiSkill)).toBe(true);
    expect(multiSkill.canReceiveCombo(forwardSkill)).toBe(true);
  });

  test("an attack-all skill can send a combo", () => {
    expect(multiSkill.canSendCombo(forwardSkill)).toBe(true);
    expect(forwardSkill.canReceiveCombo(multiSkill)).toBe(true);
  });
  ...
  test("can be created with a json object", () => {
    ...
    expect(jsonSkill.receives).toStrictEqual(
      Set([ComboTypes["Dead Stop"], ComboTypes.Move])
    );
    expect(jsonSkill.attackAll).toBeFalsy();
  });
  ...
  test("errors on invalid receives combotypes", () => {
    expect(() => {
      new Skill("badSkill", "badSkill", "Sword", [], ["HotDog"], false);
    }).toThrow("Unknown combo type in receives array. receives: " + ["HotDog"]);
  });
  ...
  test("A skill that would self combo but is an attack-all skill should not self combo", () => {
    expect(multiSkill.canSelfCombo()).toBeFalsy();
  });

  test("PopKnight should not self combo because it is attack-all", () => {
    expect(
